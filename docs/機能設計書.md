...
Atomic Design を採用する。UI の粒度を5階層で管理し、再利用性と責務の明確さを保つ。

### 階層の定義

| 階層 | 役割 | 判断基準 |
|---|---|---|
| **Atoms** | 最小単位のUI部品。それ以上分解できない | 単体で意味を持つ最小のHTML要素に近いもの |
| **Molecules** | Atoms を組み合わせた小さな部品 | 複数の Atoms が協調して1つの役割を果たすもの |
| **Organisms** | 機能を持つ複合UI部品 | データの取得・操作を意識した、ページの一部を構成するもの |
| **Templates** | ページのレイアウト骨格 | コンテンツを持たず、配置だけを担うもの |
| **Pages** | 実際のページ | Next.js App Router の `page.tsx`（自動対応） |
...
```
frontend/src/
├── app/                                    # Pages 層（Next.js App Router）
│   ├── page.tsx                            # ログインページ（/）
│   └── (authenticated)/
│       ├── dashboard/page.tsx              # ダッシュボード（/dashboard）
│       ├── weight/page.tsx                 # 体重記録ページ（/weight）
│       ├── meal/page.tsx                   # 食事記録ページ（/meal）
│       └── exercise/page.tsx              # 運動記録ページ（/exercise）
│
└── components/
    ├── atoms/                              # 最小単位のUI部品
    ...
    └── templates/                          # ページのレイアウト骨格
        └── AuthenticatedLayout.tsx         # 認証済みページ共通レイアウト
```
...
- 上位層は下位層を使える（Pages は Organisms を使える）
- 下位層は上位層を使ってはいけない（Atoms は Molecules を使えない）
- 同一層内での参照は原則禁止
